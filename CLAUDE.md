...
- **A message gives up every posting it links to.** `max_urls_per_message` stopped at the first 12, so a job24 digest of 20 jobs lost eight of them every run — invisibly, because a mail yielding 12 leads reads exactly like a mail that had 12 jobs. Over 25 reports, 20 messages ended on exactly 12 against 5 on 11, which is the shape of a cap rather than of mail. It is deleted, like `max_pages_per_run` before it (`docs/adr/0001`): a cap that truncates a list does not sample it, it starves whatever sorts last. What bounds the work instead is `allows_url`, which admits only a provider's own posting URLs, and `resolve_redirect`, which makes a request only for a tracker host and only for `MAX_REDIRECT_HOPS` of them — so footer and nav junk is refused rather than counted, however much of it a mail carries.
